...
function cmd_op( $params )
{
	...
	if ( $this->uid != $rooms[$rid][founderuid] && empty( $roomaop[$rid][$this->uid] ) && !$this->roomop[$rid] ) return 'У Вас нет таких прав';
	if ( !empty( $roomaop[$rid][$uid] ) ) return '['.$params[1].'] является аопом';
	if ( !isset( $uid2sock[$uid] ) ) return '['.$params[1].'] на данный момент не в чате';
	...
	if ( $conns[$uid2sock[$uid]]->rid == $rid )
		...
		$this->send( '['.$this->login.'] '.( $o ? 'дал' : 'снял' ).' Вам статус op в комнате '.$rooms[$rid][caption], $uid, 0 );
	$GLOBALS[aeval][] = '$this->send(\'['.$this->login.'] '.( $o ? 'дал' : 'снял' ).' статус op для ['.$params[1].']\',0,0);';
}
