...
{
	if ($GLOBALS[this]->antimat($params[1])) {
		$GLOBALS[aeval][] = '$this->send("Сообщение не может быть отображено публично",'.$GLOBALS[this]->uid.',0);';
		$GLOBALS[aeval][] = '$this->send("'.$params[1].'",'.$GLOBALS[this]->uid.');';
	} else $GLOBALS[aeval][] = '$this->send("'.$params[1].'",0,false,true);';
}
