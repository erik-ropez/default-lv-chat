...
function cmd_rooms( $params )
{
	global $rooms;
	$b = array();
	foreach ( $rooms as $i => $a )
	{
		$s = $i.'. '.$a[caption].', '.date( 'Y-m-d', $a[created] );
		if ( $a[founderuid] )
		{
			$s .= ' - '.date( 'Y-m-d', $a[created] + ( $a[credits] - 1 ) * 86400 ).', '.$a[founderlogin];
			if ( $a[onlyop] ) $s .= ', закрытая';
		}
		$b[] = $s;
	}
	return implode( '<br>', $b );
